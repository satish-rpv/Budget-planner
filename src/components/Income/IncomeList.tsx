...
import { Trash2, Filter, ArrowUpCircle } from 'lucide-react';
import { IncomeSource } from '../../types/budget';

const IncomeList: React.FC = () => {
  const { budget, removeIncome } = useBudget();
  ...
  const filteredIncome = budget.income
    .filter(income => filterSource === 'All' || income.source === filterSource)
    .sort((a, b) => {
      if (sortBy === 'date') {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
        <h2 className="text-xl font-bold text-gray-900 mb-4 sm:mb-0">Your Income</h2>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex items-center">
            ...
            <select
              value={filterSource}
              onChange={(e) => setFilterSource(e.target.value as IncomeSource | 'All')}
      ...
      {filteredIncome.length > 0 ? (
        ...
        <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-lg">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">No income found</h3>
          <p className="text-gray-500">
            {filterSource === 'All' 
              ? "You haven't added any income yet." 
              : `No income from the ${filterSource} source.`}
          </p>
        </div>
      )}
    </div>
